...
//    system("pause");
    ...
    x_active = 0;
    y_active = 0;
    field_observer = new FieldObserver;
...
void Field::moveUp(int step, Player &player, Log &log) {
    ...
    y_active = (height + y_active - step) % height;//зацикливание поля
    field_observer->changedPlayerPosition(log, x_active, y_active);
    array[y_active][x_active].activeReaction();
