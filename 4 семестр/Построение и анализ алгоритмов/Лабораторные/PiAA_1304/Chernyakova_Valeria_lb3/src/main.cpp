...
class GraphMatrix {
    ...
    public:
        ...
        // Принимает на вход текущую вершину и путь, матрицу возможности посещения вершин
        ...
        // Обновление переменных класса в соответствии с происходящими изменениями внутри метода
...
std::vector<std::vector<int>> readMatrixFile() {
    ...
    while (getline(file_input, line)) {
        ...
        line = std::regex_replace(line, std::regex(infinity_string), number_to_replace);
        line = std::regex_replace(line, std::regex(dash_string), number_to_replace);
