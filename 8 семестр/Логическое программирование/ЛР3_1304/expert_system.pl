...
% Если по данному симптому уже был получен ответ, используем его (не задаём повторно).
% Иначе, выводим вопрос и сохраняем ответ.
ask(Symptom) :-
    known(Symptom, _), !.
ask(Symptom) :-
    format('U vas est simptom "~w"? (da/net): ', [Symptom]), nl,  % Выводим вопрос
    read(Response),                 % Считываем ответ пользователя
    asserta(known(Symptom, Response)).  % Сохраняем ответ динамически
